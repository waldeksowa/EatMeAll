    ...
    <div class="">
        <div class="full-width bg-primary" style="min-height:20vh; height:150px">
          <div class="row q-pa-lg justify-end q-gutter-sm">
            ...
            <q-btn class="bg-white" label="Wygeneruj Obecny tydzień" @click="createScheduleForWeek()"></q-btn>
            <q-btn class="bg-white" label="Kalendarz"></q-btn>
          </div>
        </div>
    </div>
    <div v-if="areMealsWasFeached()" class="row justify-center q-py-md no-wrap mobile-column col-12">
      <div class="col-grow col-shrink q-gutter-sm" v-for="(s,j) in mealsSchedule" :key="j">
        <div class="text-center text-h5">{{returnCorrectDayLabel(s.day)}}</div>
        <div class="q-gutter-sm">
          <div class="" v-for="(d,i) in s.meal" :key="i">
            ...
            <q-card class="my-card-min" >
              ...
              <q-img :src="returnImgByMealTime(s.date[i])" class="card-img" @click="showDeatilDialog(d)">
      ...
      <q-dialog v-model="isScheduleShow">
        <createScheduleDialog @fetchScheduledata="fetchScheduledata()"></createScheduleDialog>
      </q-dialog>
    </div>
  </q-page>
</template>
...
export default {
  ...
  components:{
    ...
    'createScheduleDialog': require('../components/meals/CreateNewSchedule.vue').default,
  }
}
