    ...
    <div class="q-gutter-y-md  q-col-gutter-md q-pa-md fit row wrap justify-center items-start content-start">
      <div class="col-grow col-sm-9">
        <q-form class="q-col-gutter-y-lg">
          ...
          <q-input filled class="" v-model.trim="recipe.name" label="Nazwa Posiłku" lazy-rules
                   :rules="[val => val && val.length > 0 || 'To pole jest wymagane']"/>
          <q-input filled type="number" v-model.number="recipe.time" label="Czas Przygotowania (min)" lazy-rules
                   :rules="[val => !!val || 'To pole jest wymagane']"/>
          <q-input filled v-model.trim="recipe.author" label="Autor Przepisu" lazy-rules
                   :rules="[val => !!val || 'To pole jest wymagane']"/>
          <q-input filled v-model.trim="recipe.description" label="Dodatkowy Opis" lazy-rules
                   :rules="[val => !!val || 'To pole jest wymagane']"/>
          ...
          <div class="q-pa-lg">
            ...
            <div class="row justify-between">
              <q-checkbox class="desktop-only text-body1" v-for="(box,index) in checkboxOptions" :key="index" size='lg'
                          v-model="box.checked" :label="box.label"/>
            </div>
          </div>
          <div class="text-h4  text-center">Kroki Przygotowania</div>
          <q-checkbox v-model="areStepsHiden" label="Brak krokow przygotwania" class="text-body1" size='lg'
                      @click="!areStepsHiden"/>
          <div class="q-col-gutter-y-lg">
            <div v-for="(row,index) in recipe.steps" :key="index">
              <div>
                <q-input filled v-model.trim="row.name" type='text' :label="returnStepLabelWithNumber(index + 1)"
                         :disable="areStepsHiden" :rules="[val => !!val || 'To pole jest wymagane']">
                  <template v-slot:after>
                    <q-btn flat class="bg-primary full-height text-white btn-h" icon="remove"
                           @click="deleteRow(index)"></q-btn>
                  </template>
                </q-input>
              </div>
            </div>
            <div class="q-mb-lg">
              <q-btn flat class="bg-primary full-width text-white btn-h" icon="receipt_long" label="Dodaj Krok"
                     @click="addRow()"></q-btn>
            </div>
          </div>

          <div class="q-col-gutter-y-lg q-mt-lg">
            <div class="text-h4 text-center"> Posiłek Zawiera</div>
            <div class=" text-body1 ">Kcal ({{ calcMealCalorific() }})Kcal</div>
            <div class="text-body1 ">Węglowodany ({{ calcMealCarbohydrates() }}/{{ calcMealWeight() }})g</div>
            <q-linear-progress stripe rounded size="20px" :value="calcCarbohydratesPercentage()" color="red" class="q-mt-sm"/>
            <div class="text-body1 ">Tłuszcze ({{ calcMealFat() }}/{{ calcMealWeight() }})g</div>
            <q-linear-progress stripe rounded size="20px" :value="calcFatPercentage()" color="green" class="q-mt-sm"/>
            <div class="text-body1 ">Białko ({{ calcMealProtein() }}/{{ calcMealWeight() }})g</div>
            <q-linear-progress stripe rounded size="20px" :value="calcProteinPercentage()" color="blue"
                               class="q-mt-sm"/>
          </div>
          <div class="text-h4 text-center q-mb-lg">Dodaj Produkt</div>
          <div>
            <q-btn color="primary full-width btn-h" label="Dodaj Produkty" @click="dialog = !dialog"
              ...
              <div v-for="(p,i) in recipe.addedProducts" :key="i">
                <q-card class="my-card full-width" flat bordered>
                  <q-item class="row">
                    ...
                    <q-item-section class="col-sm-3">
                      <div class="mobile-only">
                        <q-input filled type='number' class="text-capitalize bg-white full-width"
                                 v-model.number="p.amount" label="Ilosc" suffix="(G)"
                                 :rules="[val => !!val || 'To pole jest wymagane']"/>
                        <q-btn flat class="bg-primary text-white full-width btn-h"
                               :icon='includeProduct(p.product) ? "remove" : "add"'
                               @click='includeProduct(p.product) ?  removeProducts(p.product) : addProductToList(p.product)'></q-btn>
                      </div>
                  ...
                  <q-item-section class="q-pa-sm bg-grey-3">
                    <div class="text-subtitle2 flex ">Kalorie: {{ p.product.calorific }}(Kcal) Tłuszcze:
                      {{ p.product.fat }}(g) Weglowodany: {{ p.product.carbohydrates }}(g) Bialko:
                      {{ p.product.protein }}(g)
                    </div>
                  </q-item-section>
                </q-card>
      ...
      <q-card style="width:80vw;height:95vh">
        <q-card-section class="bg-secondary text-center">
          ...
          <div class="row inline full-width justify-center q-mb-md">
            <q-select filled class="text-capitalize bg-white col col-auto col-grow" v-model="sort" label="Sortuj"
                      :options="returnSortLabel()"/>
            <q-select filled class="text-capitalize bg-white col col-auto col-grow" v-model="dropdownType"
                      label="Kategoria" :options="returnProductsTypes()"/>
          </div>
        ...
        <q-tab-panels v-model="dropdownType" animated swipeable infinite class="bg-white shadow-2">
          <q-tab-panel v-for="(typeName,index) in returnProductsTypes()" :key="index" :name="typeName"
                       :label="typeName">
            <q-card-section>
                ...
                <q-card class="my-card full-width" flat bordered v-if="showProductsToFind(p)">
                  <div>
                    <q-item>
                      ...
                      <q-btn flat class="bg-primary self-center text-white btn-h"
                             :icon='includeProduct(p) ? "remove" : "add"'
                             @click=' includeProduct(p) ?  removeProducts(p) : addProductToList(p)'></q-btn>
                    </q-item>
                    <q-separator/>
                    <q-item-section class="q-pa-sm bg-grey-3">
                      <div class="text-subtitle2 flex ">Kalorie: {{ p.calorific }}(Kcal) Tłuszcze: {{ p.fat }}(g)
                        Weglowodany: {{ p.carbohydrates }}(g) Bialko: {{ p.protein }}(g)
                      </div>
                    </q-item-section>
                  </div>
                </q-card>
...
export default {
  ...
  methods: {
    ...
    calcMealCalorific() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.calorific / 100).reduce((a, b) => a + b, 0).toFixed(0);
    },
    calcMealWeight() {
      return this.recipe.addedProducts.map(p => p.amount).reduce((a, b) => a + b, 0).toFixed(0);
    },
    calcMealCarbohydrates() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.carbohydrates / 100).reduce((a, b) => a + b, 0).toFixed(2);
    },
    calcMealFat() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.fat / 100).reduce((a, b) => a + b, 0).toFixed(2);
    },
    calcMealProtein() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.protein / 100).reduce((a, b) => a + b, 0).toFixed(2);
    },
    calcFatPercentage() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.fat / 100).reduce((a, b) => a + b, 0) * 9 / this.calcMealCalorific();
    },
    calcProteinPercentage() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.protein / 100).reduce((a, b) => a + b, 0) * 4 / this.calcMealCalorific();
    },
    calcCarbohydratesPercentage() {
      return this.recipe.addedProducts.map(p => p.amount * p.product.carbohydrates / 100).reduce((a, b) => a + b, 0) * 4 / this.calcMealCalorific();
    },
  ...
  created() {
    ...
    fetch(ALL_PRODUCTS).then(response => response.json()).then(data => {
      this.products = data;
    });
  },
  // computed:{
