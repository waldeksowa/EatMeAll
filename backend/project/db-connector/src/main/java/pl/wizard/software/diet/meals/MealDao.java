...
public interface MealDao extends JpaRepository<MealEntity, Long> {
    ...
    @Query(value = "select ml.*, mt.*, st.*, mp.*, pr.*, mm.*, mb.*, sh.*, ep.*, epr.*, pml.*, pmt.*, pst.*, pmp.*, ppr.*, 1 as clazz_ " +
            " from meals ml " +
    ...
    public List<MealEntity> findRandomByMealTimeTest1(int mealTime, int amount);
    ...
    @Query(value = "select m.* " +
            ...
            " order by random() " +
            " limit ?2 ", nativeQuery = true)
    public List<MealEntity> findRandomByMealTimeTest2(int mealTime, int amount);
}
